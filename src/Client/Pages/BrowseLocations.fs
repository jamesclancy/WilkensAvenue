...
let renderNeighborhoodInformation (locationSummaryViewModel: LocationSummaryViewModel) (dispatch: Msg -> unit) =
   [
        br [ ]
        div [] [ str locationSummaryViewModel.Neightborhood ]
   ]
...
let leftMenu =
  Bulma.panel [
      Bulma.panelHeading [ prop.text "Filter" ]
      ...
      Bulma.panelBlock.div [
          Bulma.control.div [
              prop.children [
                  TagsInput.input [
                      tagsInput.placeholder "Filter to Tags"
                      ...
                      tagsInput.autoCompleteSource (
                          (fun text ->
                              async {
                                  do! Async.Sleep 10
                                  return ["Railroad";"Steel";"Canning"] |> List.filter (fun x -> x.ToLowerInvariant().Contains(text.ToLowerInvariant()))
                              }
                          )
                      )
                  ]
      ...
      Bulma.panelBlock.div [
          Bulma.control.div [
              prop.children [
                  TagsInput.input [
                      tagsInput.placeholder "Filter to categories"
                      tagsInput.defaultValue [  ]
                      tagsInput.onTagsChanged (fun x -> Fable.Core.JS.console.log(x))
                      tagsInput.allowOnlyAutoCompleteValues true
                      tagsInput.caseSensitive false
                                ...
                                div [ Class "columns is-multiline" ]
                                    [
                                          ...
                                          div [ Class "column is-12-mobile is-6-tablet is-9-desktop" ]
                                              [
                                                  div [] [ b [] [ str "150 exciting locations found..."] ]
                                                  div [ Class "container" ]
                                                        [
                                                          section [ Class "section" ]
                                                           [ 
