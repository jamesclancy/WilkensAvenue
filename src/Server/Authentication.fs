...
let authChallenge : HttpFunc -> HttpContext -> HttpFuncResult =
    requiresAuthentication (Auth.challenge Microsoft.AspNetCore.Authentication.OpenIdConnect.OpenIdConnectDefaults.AuthenticationScheme)
...
let openIdConfig  (services : IServiceCollection) =
  ...
  let clientId = config.GetValue<string>("OpenIDOAuthClientId")
  let tenantId = config.GetValue<string>("OpenIDTenantId")
  let secret = config.GetValue<string>("OpenIDOAuthKey")
  let authority = config.GetValue<string>("OpenIDAuthority")
  let responseType = config.GetValue<string>("OpenIDResponseType")
